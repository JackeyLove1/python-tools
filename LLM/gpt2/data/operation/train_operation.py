...
block_size = max_prompt_len + 1 + max_resp_len  # 141
...
train_data = np.memmap(os.path.join(data_dir, f'{sft_train}.bin'),
                       dtype=np.uint16, mode='r')
val_data = np.memmap(os.path.join(data_dir, f'{sft_val}.bin'),
                     dtype=np.uint16, mode='r')
...
def get_batch(split):
    ...
    # Example.
    # prompt: p0, p1, p2; response: r0, r1, r2, r3; concatenated sequence: p0, p1, p2, [EQUAL], r0, r1, r2, r3, [EOS]
    # masked x: [EQUAL], r0, r1, r2, r3
...
def get_val_hit_ratio_batch():
    ...
    x = torch.stack(
        [torch.from_numpy((data[i:i + max_prompt_len]).astype(np.int64)) for i in ix])  # (bs, max_prompt_len)
...
def remove_tokens(ids, token: int):
...
def calc_val_hit_ratio():
    ...
    for idx in range(total_count):
        ...
        if predict_output == correct_output:
            hit_count += 1
...
while True:
    ...
    if iter_num != 0 and iter_num % eval_interval == 0 and master_process:
        ...
        print(
            f"validation: step {iter_num}: time {dt * 1000:.2f}ms, train loss {losses['train']:.4f}, val loss {losses['val']:.4f}, "
    ...
    # logger.info(f"train/grad_norm:{total_norm}")
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        # logger.info(f"train/lr:{lr}, train/loss:{lossf},train/mfu:{running_mfu * 100},train/time:{dt * 1000}")
    iter_num += 1
    local_iter_num += 1
