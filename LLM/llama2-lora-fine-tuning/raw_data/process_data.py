...
def format_story_to_json(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    datas = []
    half_length = min(len(content) // 2, 500 // 2)
    for idx in range(0, len(content), half_length*2):
        input_text = content[idx:idx+half_length]
        output_text = content[idx+half_length:idx+half_length*2]
        data = {
            "instruction": "你是个优秀的小说作家，请续写下面的内容：",
            "input": input_text,
            "output": output_text
