...
class Vocab:
    ...
    def __init__(self, token_dict: List[str], specials):
        self.stoi = {v[0]: (k, v[1]) for k, v in enumerate(token_dict)}
        self.itos = {k: (v[0], v[1]) for k, v in enumerate(token_dict)}
        ...
        if isinstance(token, (int, np.int64)):
            ...
            for t in token:
                if t in self.itos:
                    ...
                    raise ValueError(f"Token {t} is not a valid token")
            return res
